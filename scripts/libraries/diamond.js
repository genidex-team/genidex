...
function getSelectors(contract) {
  ...
  const selectors = signatures.reduce((acc, val) => {
    if (val !== 'init(bytes)') {
      acc.push(ethers.id(val).slice(0, 10));
    }
    return acc;
  }, []);
...
function getFunctionNameFromSelector(abi, selector, format='full') {
  const iface = new ethers.Interface(abi);
  for (const fragment of Object.values(iface.fragments)) {
    // console.log(fragment)
    if(fragment.type == 'constructor') continue;
    const sigHash = ethers.id(fragment.format()).slice(0, 10); // 4 bytes
...
function formatFacets(abi, facets, format='full'){
  var result = {};
  for(var i in facets){
    var facet = facets[i];
    var [facetAddr, selectors] = facet;
    var functions = {};
    for(var j in selectors){
      var selector = selectors[j];
      functions[selector] = getFunctionNameFromSelector(abi, selector, format);
    }
    result[facetAddr] = functions;
  }
  return result;
}
